...
class ServiceController extends Controller
{
        ...
        if (!empty($request->keyword)) {
            $query->where(function ($q) use ($request) {
                $q->where('services.name', 'like', '%' . $request->keyword . '%')
                    ->orWhere('services.name_ar', 'like', '%' . $request->keyword . '%')
    ...
    */
    ...
    public function save(Request $request)
    {
        /*
        |--------------------------------------------------------------------------
        | Sync category / sub-category
        |--------------------------------------------------------------------------
        */

        if (!$request->filled('category') && $request->filled('category_ar')) {
            $request->merge([
        ...
        if (
            !$request->filled('sub_category')
            && $request->filled('sub_category_ar')
        ) {
            $request->merge([
                'sub_category' => $request->sub_category_ar
            ]);
        }
        ...
        */
        ...
        $service->short_desc_ar = $request->short_description_ar;
        ...
        */
        ...
        $service->meta_description =
            ...
            ?: 'EXPLORE ' . $request->name .
            ' FROM Marhaba Ai, PROVIDING TOP EVENT SERVICES IN DUBAI.';

        $service->meta_description_ar =
            $request->meta_description_ar
            ?: $request->name_ar .
            ' من Marhaba Ai، نقدم أفضل خدمات الفعاليات في دبي.';
        ...
        $service->meta_keywords_ar =
            ...
            ?: 'Marhaba Ai, ' . $request->name_ar . ', دبي, الإمارات';
    ...
    */
    ...
    public function removeGalleryImage(
        ...
        $galleryImages =
            is_array($service->gallery_images)
            ? $service->gallery_images
            : (
                json_decode(
                    $service->gallery_images,
