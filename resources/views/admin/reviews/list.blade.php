                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="card-tools">
                            <form action="{{ route('reviewList') }}" method="get">
                                <div class="input-group mb-0 mt-0" style="width: 700px;">
                                    ...
                                    <select name="reviews" class="form-control">
                                        ...
                                        @foreach($reviews as $reviewItem)
                                            <option
                                                value="{{ $reviewItem->id }}"
                                                {{ request()->get('reviews') == $reviewItem->id ? 'selected' : '' }}
                                            >
                                                {{ $reviewItem->name }}
                                            </option>
                                        @endforeach
                                    ...
                                    <a href="{{ route('reviewList') }}" class="btn btn-secondary ml-2">Reset</a>
                                </div>
                            </form>
                        </div>
                    </div>
                                ...
                                @if(!empty($reviews) && count($reviews) > 0)
                                    ...
                                    @foreach($reviews as $review)
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if(!empty($review->image) && file_exists(public_path('uploads/reviews/thumb/large/' . $review->image)))
                                                    <img
...
function deleteReview(id) {
    if (confirm("Are you sure you want to delete?")) {
        $.ajax({
            url: '{{ route("review.delete", ":id") }}'.replace(':id', id),
            type: 'POST',
            dataType: 'json',
            data: {
